...
### Script: TCGA_Clinical_Variable_pEMT_Score_Correlations.R
### Source Script Name: 24-1205_TCGA_Survival_Extrathyroidal_Extension_pEMT.R

### Goal: 
# Correlate clinical variables (e.g., extrathyroidal extension, LN mets, distant mets, location) with pEMT ssGSVA score in VUMC/UW bulk RNA-sequencing data

# Adapted from 24-1130_TCGA_Survival_Extrathyroidal_Extension_myCAFs.R
# Using Puram et al. pEMT scores (100 genes from Puram et al.)
...
TCGA_Puram_pEMT_Deconvolution <- readRDS(file = "data_in_use/deconvolution_scores/24-1205_TCGA_Puram_pEMT_GSVA_Scores.RDS")
...
TCGA_Puram_pEMT_Deconvolution$TCGA_ID <- gsub("\\.", "-", TCGA_Puram_pEMT_Deconvolution$TCGA_ID)
TCGA_Puram_pEMT_Deconvolution <- TCGA_Puram_pEMT_Deconvolution %>% dplyr::rename("SAMPLE_ID" = "TCGA_ID")

Patient_Sample_Combined <- Patient_Sample_Combined %>% merge(TCGA_Puram_pEMT_Deconvolution)
rm(TCGA_Puram_pEMT_Deconvolution)
...
# Run stats 
#res.aov <- aov(pEMT ~ DISEASE_STAGE, data = Disease_Stage_Tibble)
#summary(res.aov)
#TukeyHSD(res.aov)
# Make the plot with restricted groups
plot <- ggplot(Disease_Stage_Tibble, aes(DISEASE_STAGE_Simplified, pEMT)) + 
...
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_Disease_Stage.png",
       plot, create.dir = TRUE,
       width = 8, height = 7, dpi = 600)

# Stats for pEMT score by Disease Stage
kruskal.test(pEMT ~ DISEASE_STAGE_Simplified, data = Disease_Stage_Tibble) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Disease_Stage_Tibble$pEMT, Disease_Stage_Tibble$DISEASE_STAGE_Simplified, # this returned 0.2475
                     p.adjust.method = "BH")
...
################# RISK GROUP #####################
## Risk Group light
# Risk groups with pEMT plotted
plot <- ggplot(Patient_Sample_Combined, aes(RISK_GROUP, pEMT)) + 
  geom_boxplot(outlier.size = -1,
               aes(fill = RISK_GROUP),
               alpha = 0.4,
               show.legend = FALSE) + 
...
plot <- ggplot(Patient_Sample_Combined_RISK, aes(RISK_GROUP, pEMT)) + 
  geom_boxplot(outlier.size = -1,
               aes(fill = RISK_GROUP),
               alpha = 0.4,
               show.legend = FALSE) + 
  ...
  theme(
        ...
        axis.text = element_text(size = 25, color = "black", face = "bold"))
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_Risk_Group.png",
       plot, create.dir = TRUE,
       width = 8, height = 7, dpi = 600)

# Stats for pEMT score by Risk group
kruskal.test(pEMT ~ RISK_GROUP, data = Patient_Sample_Combined_RISK) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Patient_Sample_Combined_RISK$pEMT, Patient_Sample_Combined_RISK$RISK_GROUP, # this returned 0.2475
                     p.adjust.method = "BH")
...
plot <- ggplot(Patient_Sample_Combined_RISK, aes(RISK_GROUP_SIMPLIFIED, pEMT)) + 
  geom_boxplot(outlier.size = -1,
...
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_Risk_Group_simplified.png",
       plot, 
       width = 3, height = 3, dpi = 600)

# stats
kruskal.test(pEMT ~ RISK_GROUP_SIMPLIFIED, data = Patient_Sample_Combined_RISK) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Patient_Sample_Combined_RISK$pEMT, Patient_Sample_Combined_RISK$RISK_GROUP_SIMPLIFIED, # this returned 0.2475
                     p.adjust.method = "BH")
...
plot <- ggplot(Patient_Sample_Combined, aes(EXTRATHYROIDAL_EXTENSION, pEMT)) + 
  geom_boxplot(outlier.size = -1,
...
plot <- ggplot(Extrathyroidal_Stats, aes(Extrathyroidal_Simplified, pEMT)) + 
  geom_boxplot(outlier.size = -1,
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", size = 16)) 
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_Extrathyroidal_simplified.png",
       plot, 
       width = 3, height = 3, dpi = 600)

# Stats for pEMT score by Extrathyroidal Extension
kruskal.test(pEMT ~ Extrathyroidal_Modified, data = Extrathyroidal_Stats) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Extrathyroidal_Stats$pEMT, Extrathyroidal_Stats$Extrathyroidal_Modified, # this returned 0.2475
                     p.adjust.method = "BH")

# Simplified
kruskal.test(pEMT ~ Extrathyroidal_Simplified, data = Extrathyroidal_Stats) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Extrathyroidal_Stats$pEMT, Extrathyroidal_Stats$Extrathyroidal_Simplified, # this returned 0.2475
                     p.adjust.method = "BH")
...
# Here I will look at histological type and how it compares to both the pEMT score and how it compares to BRS
...
# Histological Type
plot <- ggplot(Histology_Plot_Cohort, aes(HISTOLOGICAL_TYPE_COMPLEX, pEMT)) + 
  geom_boxplot(outlier.size = -1,
...
plot <- ggplot(Histology_Plot_Cohort, aes(HISTOLOGICAL_TYPE_SIMPLIFIED, pEMT)) + 
  geom_boxplot(outlier.size = -1,
  ...
  geom_jitter(aes(),
              ...
              alpha = c(0.8),
              show.legend = FALSE) +
  # Changed colors to match BRS plot (BRAF/RAS score plot)
  scale_fill_manual(values = c("#FFCC99", "grey60", "indianred1")) + 
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", size = 16)) 
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_TCGA_Histology_Breakdown_by_pEMT.png",
       plot, 
       width = 4, height = 3, dpi = 600)

# Stats for pEMT score by HISTOLOGICAL_TYPE_SIMPLIFIED
kruskal.test(pEMT ~ HISTOLOGICAL_TYPE_SIMPLIFIED, data = Histology_Plot_Cohort) # shouldn't be necessary for only two groups
pairwise.wilcox.test(Histology_Plot_Cohort$pEMT, Histology_Plot_Cohort$HISTOLOGICAL_TYPE_SIMPLIFIED, # this returned 0.2475
                     p.adjust.method = "BH")
...
plot <- ggplot(LN_Plot_Cohort, aes(LN_Binary, pEMT)) + 
  geom_boxplot(outlier.size = -1,
               aes(fill = LN_Binary),
               alpha = 0.2,
               show.legend = FALSE) + 
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", size = 16)) 
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_LN_Spread.png",
       plot, 
       width = 3, height = 3, dpi = 600)

# LN stats
kruskal.test(pEMT ~ LN_Binary, data = LN_Plot_Cohort) # shouldn't be necessary for only two groups
pairwise.wilcox.test(LN_Plot_Cohort$pEMT, LN_Plot_Cohort$LN_Binary, #
                     p.adjust.method = "BH")
# Cleaning up from extrathyroidal extension
rm(LN_Plot_Cohort)
...
DM_Plot_Cohort <- Patient_Sample_Combined %>% subset(PATH_M_STAGE == "M0" |
                                                     PATH_M_STAGE == "M1")
# Plot binary category and perform stats
plot <- ggplot(DM_Plot_Cohort, aes(PATH_M_STAGE, pEMT)) + 
  geom_boxplot(outlier.size = -1,
  ...
  theme(
    ...
    axis.text.y = element_text(face = "bold", size = 16)) 
plot
ggsave("outputs/24-1205_pEMT_TCGA_Survival_Extrathyroidal/24-1205_pEMT_by_Distant_Met_Spread.png",
       plot, 
       width = 3, height = 3, dpi = 600)

# DM stats
kruskal.test(pEMT ~ PATH_M_STAGE, data = DM_Plot_Cohort) # shouldn't be necessary for only two groups
pairwise.wilcox.test(DM_Plot_Cohort$pEMT, DM_Plot_Cohort$PATH_M_STAGE, #
