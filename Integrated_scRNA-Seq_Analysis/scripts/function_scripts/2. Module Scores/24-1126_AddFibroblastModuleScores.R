...
### Script: 24-1126_AddFibroblastModuleScores.R
...
### 24-1126 Update
# This is updaed from the 24-1125 version of this script
# I added Belle et al., Dinh et al., and Ma et al. fibroblast gene sets
...
AddFibroblastScores <- function(SO){
  ...
  SO$Iesato_Pericyte_Mod_Score1 <- NULL
  ...
  SO$Iesato_Fibroblast_Mod_Score1 <- NULL
  ...
  ### Belle et al. CAF subsets

  # Belle et al. apCAF subset
  Belle_apCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Belle_etal_apCAF.txt", header = FALSE, sep = NULL)
  Belle_apCAF_list <- list(Belle_apCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Belle_apCAF_list,
                                      name = "Belle_apCAF_Mod_Score")
  rm(Belle_apCAF, Belle_apCAF_list)

  # Belle et al. iCAF subset
  Belle_iCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Belle_etal_iCAF.txt", header = FALSE, sep = NULL)
  Belle_iCAF_list <- list(Belle_iCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Belle_iCAF_list,
                                      name = "Belle_iCAF_Mod_Score")
  rm(Belle_iCAF, Belle_iCAF_list)

  # Belle et al. myCAF1 subset
  Belle_myCAF1 <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Belle_etal_myCAF1.txt", header = FALSE, sep = NULL)
  Belle_myCAF1_list <- list(Belle_myCAF1$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Belle_myCAF1_list,
                                      name = "Belle_myCAF1_Mod_Score")
  rm(Belle_myCAF1, Belle_myCAF1_list)

  # Belle et al. myCAF2 subset
  Belle_myCAF2 <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Belle_etal_myCAF2.txt", header = FALSE, sep = NULL)
  Belle_myCAF2_list <- list(Belle_myCAF2$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Belle_myCAF2_list,
                                      name = "Belle_myCAF2_Mod_Score")
  rm(Belle_myCAF2, Belle_myCAF2_list)

  # Belle et al. myCAF3 subset
  Belle_myCAF3 <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Belle_etal_myCAF3.txt", header = FALSE, sep = NULL)
  Belle_myCAF3_list <- list(Belle_myCAF3$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Belle_myCAF3_list,
                                      name = "Belle_myCAF3_Mod_Score")
  rm(Belle_myCAF3, Belle_myCAF3_list)

  ### Ma et al. CAF subsets

  # Ma et al. iCAF subset
  Ma_iCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_iCAF.txt", header = FALSE, sep = NULL)
  Ma_iCAF_list <- list(Ma_iCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_iCAF_list,
                                      name = "Ma_iCAF_Mod_Score")
  rm(Ma_iCAF, Ma_iCAF_list)

  # Ma et al. mCAF subset
  Ma_mCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_mCAF.txt", header = FALSE, sep = NULL)
  Ma_mCAF_list <- list(Ma_mCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_mCAF_list,
                                      name = "Ma_mCAF_Mod_Score")
  rm(Ma_mCAF, Ma_mCAF_list)

  # Ma et al. meCAF subset
  Ma_meCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_meCAF.txt", header = FALSE, sep = NULL)
  Ma_meCAF_list <- list(Ma_meCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_meCAF_list,
                                      name = "Ma_meCAF_Mod_Score")
  rm(Ma_meCAF, Ma_meCAF_list)

  # Ma et al. pCAF subset
  Ma_pCAF <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_pCAF.txt", header = FALSE, sep = NULL)
  Ma_pCAF_list <- list(Ma_pCAF$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_pCAF_list,
                                      name = "Ma_pCAF_Mod_Score")
  rm(Ma_pCAF, Ma_pCAF_list)

  # Ma et al. Pericyte subset
  Ma_Pericyte <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_Pericyte.txt", header = FALSE, sep = NULL)
  Ma_Pericyte_list <- list(Ma_Pericyte$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_Pericyte_list,
                                      name = "Ma_Pericyte_Mod_Score")
  rm(Ma_Pericyte, Ma_Pericyte_list)

  # Ma et al. SMC subset
  Ma_SMC <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Ma_etal_SMC.txt", header = FALSE, sep = NULL)
  Ma_SMC_list <- list(Ma_SMC$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Ma_SMC_list,
                                      name = "Ma_SMC_Mod_Score")
  rm(Ma_SMC, Ma_SMC_list)

  ### Dinh et al. CAF subsets (only one to add - activated myofibroblast)

  # Dinh et al. Activated Myofibroblast subset
  Dinh_Activated_Myofibroblast <- data.table::fread("data_in_use/Fibroblast_gene_sets/24-1126_Dinh_etal_Activated_Myofibroblast.txt", header = FALSE, sep = NULL)
  Dinh_Activated_Myofibroblast_list <- list(Dinh_Activated_Myofibroblast$V1)
  SO <- SO %>% Seurat::AddModuleScore(features = Dinh_Activated_Myofibroblast_list,
                                      name = "Dinh_Activated_Myofibroblast_Mod_Score")
  rm(Dinh_Activated_Myofibroblast, Dinh_Activated_Myofibroblast_list)
